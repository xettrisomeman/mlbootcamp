...
def train(X_train, y_train, function):
    ...
    model = LogisticRegression(
        C=10, solver="saga", multi_class="multinomial", max_iter=1000)
...
df_full_train, df_test = train_test_split(
    nepali_text, test_size=0.2, random_state=1, stratify=nepali_text.label)

df_train, df_val = train_test_split(
    df_full_train, test_size=0.25, random_state=1, stratify=df_full_train.label)
